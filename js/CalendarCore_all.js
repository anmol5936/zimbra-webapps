...
function(name, params, data, buffer) {
	...
	buffer[_i++] = "</td></tr>";
	 ...
	 if (!data.appt.isDraft && data.appt.isOrganizer() && data.appt.otherAttendees) { 
			var attendeeTypes = [ZmMsg.attendees,ZmMsg.location,ZmMsg.equipmentAttendee];
	
			for (var k in attendeeTypes) {
				var ptstStatus = [];
				var displayText = "";
				var zmMsgKey;
				var attendeeType = attendeeTypes[k];
	
				if(attendeeType == ZmMsg.attendees) {
					if(data.hideAttendees) { continue; }
	
					ptstStatus = data.attendeesPtStStatus;
					displayText = data.attendeesText;
	
				} else if(attendeeType == ZmMsg.location) {
					if(data.hideLocations) { continue; }
	
					ptstStatus = data.locationPtStStatus;
					displayText = data.locationText;
	
				} else {
					if(data.hideEquipments) { continue; }
	
					ptstStatus = data.equipmentPtStStatus;
					displayText = data.equipmentText;
				}
				
	buffer[_i++] = "<tr valign='center'><td colspan='2'><div class='calendar_tooltip_attendees'><br />";
	buffer[_i++] =  AjxStringUtil.htmlEncode(attendeeType) ;
	buffer[_i++] = "</div></td></tr>";
	
					if (displayText) {
				
	buffer[_i++] = "<tr valign='top'><td colspan='2' align='left' style='padding-right:5px;'>";
	buffer[_i++] =  AjxStringUtil.htmlEncode(displayText) ;
	buffer[_i++] = "</td></tr>";
...
function(name, params, data, buffer) {
	...
	buffer[_i++] =  AjxStringUtil.htmlEncode(data.locationName) ;
	 if (data.locationDescription) { 
	buffer[_i++] = "<br />";
	buffer[_i++] =  AjxStringUtil.htmlEncode(data.locationDescription) ;
	 } 
	buffer[_i++] = "</div>";
...
function(name, params, data, buffer) {
	...
	buffer[_i++] = "</td><td><table role=\"presentation\" class=\"ZRadioButtonTable\" width=100%>";
	 if (data.isGoogleAuthConfigured) { 
	buffer[_i++] = "<tr><td width=\"5\"><input type=\"radio\" name=\"extCalType\" id=\"";
	buffer[_i++] = data["id"];
	buffer[_i++] = "_shareRadioGoogle\"></td><td align=left><label for=\"";
	buffer[_i++] = data["id"];
	buffer[_i++] = "_shareRadioGoogle\">";
	buffer[_i++] =  ZmMsg.sharedCalTitleGoogle ;
