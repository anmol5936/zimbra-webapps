...
AjxTemplate.register("briefcase.Briefcase#sharedFileView", 
function(name, params, data, buffer) {
	var _hasBuffer = Boolean(buffer);
	data = (typeof data == "string" ? { id: data } : data) || {};
	buffer = buffer || [];
	...
	buffer[_i++] = "<table><tr><td>";
	buffer[_i++] =  ZmMsg.sharedFileAccess ;
	buffer[_i++] = "</td></tr><tr><td><button id=\"";
	buffer[_i++] = data["id"];
	buffer[_i++] = "\" class=\"ZmSharedFilePreview\">";
	buffer[_i++] =  ZmMsg.sharedFileRemove ;
	buffer[_i++] = "</button></td></tr></table>";

	return _hasBuffer ? buffer.length : buffer.join("");
},
{
	"id": "briefcase.Briefcase#sharedFileView"
}, false);
